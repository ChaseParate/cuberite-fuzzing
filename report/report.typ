...
However, as we discussed more about our goals and possible avenues for approaching our project, we ended up pivoting to model-guided network fuzzing instead of direct coverage-guided fuzzing.
...
We used #link("https://boofuzz.readthedocs.io/en/stable/")[`boofuzz`] (a state-of-the-art model-guided network protocol fuzzer) to accomplish this, which was recommended by Professor Nagy.
...
#footnote[
  Because we targeted specifically the network stack, our fuzzer is technically able to fuzz the network stack of _any_ Minecraft server given they follow the Minecraft protocol, including the official Java implementation from Mojang.
...
#footnote[
  The gameplay support for 1.12.2 is a bit buggy or otherwise incompatible with the Java server, but the network protocol is fully supported.
]
...
We had to do a lot of tweaking to get this to work, as `boofuzz` does not currently support this, to our knowledge.
...
- #link("https://github.com/cuberite/cuberite/issues/5639")[Issue \#5639] was found by our fuzzer only because of the way it operates.
  Since it generates a bunch of random players to join and leave the server, it's able to trigger some kind of race condition that stops the player count from decrementing correctly when a player leaves the server after a certain number of joins/leaves in quick succession.
  This was actually a challenge during fuzzing because we had to make the max player count super high on the server to allow players to join/leave many times without getting a "server is full" error.

- #link("https://github.com/cuberite/cuberite/issues/5640")[Issue \#5640] was actually the "packet of doom" we were looking for, because it really can be triggered with just one packet.
  We found that a certain type of packet called a "Plugin Message", used to transmit arbitrary data over a channel with a given name, could crash the server by triggering an assertion error when the channel's name is very long.
  Our "packet of doom" in this case is just a compressed packet containing a 40,000 character channel name, which we assume triggers buffer overflow prevention code that aborts the server when it tries to send back an "UNREGISTER" channel message with the channel name to report the channel name as unrecognized.

== Conclusion
// Reflection, how we could improve/extend the project, etc.
Using this black-box fuzzing strategy, we were able to find a couple interesting bugs, which shows that testing the network stack of a game server like this is a viable strategy.
It might even have some reproducibility benefits over a traditional AFL harness, because it doesn't require any changes to the code and works across different languages/architectures.
...
Since our grammar-guided fuzzing technique uses a sort of state machine anyways to track which packets to send / the validity of certain packets, it could also be interesting to extend this state machine to have even more validation on the client state returned by the server.
This might slow down our fuzzing even more from the usual 10-30 executions/sec, but it could be used to catch bugs that don't crash the server but do enable cheating.
