from boofuzz import Fuzzable, Request, Block
...
def create_compressed_packet(name: str, packet_id: int, inner: Fuzzable | None, threshold: int = 0) -> Request:
    children: list[Fuzzable] = [VarInt("packet_id", packet_id)]
    if inner is not None:
        children.append(inner)
    
    packet = VarIntSized("length", children=[Block("compressed", children=children, encoder=compressed(threshold))])